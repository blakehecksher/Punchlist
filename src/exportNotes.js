...
function markersForElement(element) {
  const markers = [];
  const addMarker = (marker) => {
    if (!markers.includes(marker)) markers.push(marker);
  };

  const tagMarker = INLINE_MARKERS[element.tagName];
  if (tagMarker) addMarker(tagMarker);

  // The browser editor emits style-based formatting (e.g. execCommand("bold")
  // producing <span style="font-weight: 700">), so tag names alone lose it.
  const style = (element.getAttribute("style") || "").toLowerCase();
  if (/font-weight\s*:\s*(?:bold|[5-9]00)/.test(style)) addMarker("**");
  if (/font-style\s*:\s*italic/.test(style)) addMarker("*");
  if (/text-decoration[^;]*underline/.test(style)) addMarker("__");
  if (/text-decoration[^;]*line-through/.test(style)) addMarker("~~");

  return markers;
}

function wrapWithMarkers(inner, markers) {
  if (markers.length === 0 || !inner.trim()) return inner;
  const leading = inner.match(/^\s*/)[0];
  const trailing = inner.match(/\s*$/)[0];
  const open = markers.join("");
  const close = [...markers].reverse().join("");
  return `${leading}${open}${inner.trim()}${close}${trailing}`;
}
...
function nodeToMarkdown(node) {
  ...
  for (const child of node.childNodes) {
    ...
    result += wrapWithMarkers(nodeToMarkdown(child), markersForElement(child));
  }
  return result;
}

/**
 * Descriptions are stored as HTML with escaped entities. Parsing (rather than
 * regex-stripping) keeps formatting that carries attributes or inline styles
 * and decodes entities, so "Paint &amp; trim" copies out as "Paint & trim".
 */
