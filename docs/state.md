...
- The production build is pushed to `master` and the native GitHub Actions Pages workflow is correct.
...
- GitHub's top-level Pages API status still reports `errored` from the old legacy build, but the current deployment endpoint reports `succeed` and the public URL serves the current build.
